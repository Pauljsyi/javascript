...
function groupObjects(items) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    if (!(items[i].category.toLowerCase() in result)) {
      console.log("if statement", items[i]);
      result[items[i].category] = [items[i]];
    } else if (items[i].category.toLowerCase() in result) {
      console.log(items[i].category);
      result[items[i].category].push(items[i]);
    }
  }
  return result;
}
