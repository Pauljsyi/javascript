...
function zipArraysIntoMap(keys, values) {
  let dictionary = {};
  // create an empty dictionary/ obj
  // iterate through keys and values
  // push keys and value into dictionary
  // EDGE CASE
  // if keys and values is empty return empty dictionary
  if (keys === [] && values === []) {
    return [];
  }
  ...
  for (let i = 0; i < keys.length; i++) {
    // check if item at index is an array
    // create a loop for that array
    if (Array.isArray(keys[i])) {
      // console.log("TRUE");
      for (let j = 0; j < keys[i].length; j++) {
        dictionary[keys[i][j]] = values[i];
      }
